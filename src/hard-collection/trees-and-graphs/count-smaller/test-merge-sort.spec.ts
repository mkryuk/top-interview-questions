import { countSmaller } from "./solution-merge-sort";
...
xit("countSmaller should return [0] for nums = [-1]", () => {
  const nums = [-1];
  const result = countSmaller(nums);
  expect(result).toEqual([0]);
});

xit("countSmaller should return [0,0] for nums = [-1,-1]", () => {
  const nums = [-1, -1];
  const result = countSmaller(nums);
  expect(result).toEqual([0, 0]);
});

xit("countSmaller should return [0,7,0,5,0,3,0,1,0] for nums = [1,9,2,8,3,7,4,6,5]", () => {
  const nums = [1, 9, 2, 8, 3, 7, 4, 6, 5];
  const result = countSmaller(nums);
  expect(result).toEqual([0, 7, 0, 5, 0, 3, 0, 1, 0]);
});

xit("countSmaller should pass test 32", () => {
  const nums = [
